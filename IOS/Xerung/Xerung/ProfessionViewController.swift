...
class ProfessionViewController: UIViewController ,IndicatorInfoProvider {
    public func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "Profession")
    }
   ...
   /* func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAtIndexPath indexPath: IndexPath) {
        ...
        viewController.type = "profession"
    ...
    func findBloodGroup(){
        ...
        if (contactDB?.open())! {
            let querySQL = "SELECT count(*) as Num_row , profession FROM GroupData GROUP BY profession "
            let results:FMResultSet? = contactDB?.executeQuery(querySQL,withArgumentsIn: nil)
            
            
            
            while((results?.next()) == true){
                print(results!.string(forColumn: "profession")!)
                if results!.string(forColumn: "profession")! == "-1" {
                    groupType.append("Unknown")
                }else{
                    groupType.append(results!.string(forColumn: "profession")!)
                }
    ...
    /*func saveData(){
        ...
        if contactDB.open() {
            ...
            for i in 0 ..< data.count {
                ...
                let insertSQL = "INSERT INTO GroupData (name,number,flag,address,UID,bloodGroup,city,profession,Date) VALUES ('\(info[0])', '\(info[1])','\(info[2])', '\(info[3])', '\(info[4])','\(info[5])', '\(info[6])', '\(profession)', '\(info[8])')"
